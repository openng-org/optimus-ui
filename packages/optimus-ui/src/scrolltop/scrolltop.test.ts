...
describe('ScrollTop', () => {
    ...
    afterEach(() => {
        // Several tests below spy on properties of the real, shared `document` (e.g. `defaultView`)
        // via vi.spyOn(...).mockReturnValue(...). Unlike Jasmine's spyOnProperty, which Jasmine restores
        // automatically after every spec, Vitest does not restore spies implicitly, so a leftover spy on
        // the shared document would otherwise leak into every later test (in this file and others).
        vi.restoreAllMocks();
    });
    ...
    describe('Component Initialization', () => {
        ...
        it('should initialize with window target', () => {
            vi.spyOn(scrollTop, 'bindDocumentScrollListener').mockImplementation(() => {});
            scrollTop.ngOnInit();
    ...
    describe('Scroll Listeners', () => {
        ...
        it('should not bind listeners on server platform', () => {
            ...
            vi.spyOn(serverScrollTop.renderer, 'listen').mockImplementation(() => {});
    ...
    describe('Parent Target Functionality', () => {
        ...
        it('should monitor parent scroll', async () => {
            const checkVisibilitySpy = vi.spyOn(scrollTop, 'checkVisibility').mockImplementation(() => {});
    ...
    describe('Lifecycle Hooks', () => {
        ...
        it('should clean up on destroy for window target', () => {
            vi.spyOn(scrollTop, 'unbindDocumentScrollListener').mockImplementation(() => {});
            const overlayElement = document.createElement('div');
            scrollTop.overlay = overlayElement;
            vi.spyOn(ZIndexUtils, 'clear').mockImplementation(() => {});
        ...
        it('should call super.ngOnInit', () => {
            vi.spyOn(BaseComponent.prototype, 'ngOnInit').mockImplementation(() => {});
            scrollTop.ngOnInit();
            expect(BaseComponent.prototype.ngOnInit).toHaveBeenCalled();
        });

        it('should call super.ngOnDestroy', () => {
            vi.spyOn(BaseComponent.prototype, 'ngOnDestroy').mockImplementation(() => {});
            scrollTop.ngOnDestroy();
