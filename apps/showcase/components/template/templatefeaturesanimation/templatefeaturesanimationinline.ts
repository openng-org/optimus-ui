    ...
    template: `
        <div class="w-full py-7 rounded-xl bg-surface-50 dark:bg-surface-800 relative flex flex-col items-center justify-center">
            ...
            <div class="w-[90%] h-fit overflow-hidden relative flex mt-5 rounded-lg shadow-[0px_0px_48px_0px_rgba(0,0,0,0.08)]">
                <img class="max-h-96 w-full h-auto object-cover object-top flex" [src]="$safeNavigationMigration(inlineFeaturesData[selectedID - 1]?.src)" alt="Animation Inline Feature Image" />
            </div>
        </div>
    `,
