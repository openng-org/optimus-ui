...
export class AppDocCopyMarkdown implements OnInit {
    ...
    get githubLink(): string {
        if (this.docType() === 'page') {
            ...
            return `{{ GITHUB_REPO_URL }}/tree/master/apps/showcase/doc/${docPath}/`;
        }
        if (this.currentComponentName) {
            return `{{ GITHUB_REPO_URL }}/tree/master/apps/showcase/doc/${this.currentComponentName}/`;
        }
        return '{{ GITHUB_REPO_URL }}/tree/master/apps/showcase/';
    }
